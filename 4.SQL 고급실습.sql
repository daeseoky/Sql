...
#실습하기 4-3
SELECT * FROM `Member` WHERE `name` <> '김춘추';  #<>는 !=와 같은 뜻
SELECT * FROM `Member` WHERE `pos` IN('사원', '대리'); #`pos` IN('사원','대리')는pos='사원' or pos='대리'와 같은뜻
SELECT * FROM `Member` WHERE `name` LIKE '%신'; # 신으로 끝나는 모든것
SELECT * FROM `Member` WHERE `name` LIKE '정_'; # 정으로 시작되는 두글자
...
SELECT * FROM `Sales` WHERE `month` IN (1);


#실습하기 4-4
SELECT * FROM `Sales` ORDER BY `sale`; #order by는 오름차순
SELECT * FROM `Sales` ORDER BY `sale` DESC; #order by desc는 내림차순
SELECT * FROM `Sales`
WHERE `sale` > 50000
...
SELECT COUNT(`sale`) AS `갯수` FROM `Sales`;
SELECT COUNT(*) AS `갯수` FROM `Sales`;
SELECT SUBSTRING(`hp`, 10, 4) AS `전화번호 킅자리` FROM `Member`;

#확인문제1. 2018년 1월 매출의 총합을 구하시오
SELECT SUM(`sale`) FROM `Sales` WHERE `year`=2018 AND `month`=1;


#확인문제2. 2019년 2월달에 5만원이상 매출에 대한 총합과 평균을 구하시오
SELECT
	SUM(`sale`) AS `총합`,
	AVG(`sale`) AS `평균`
FROM
...
SELECT *FROM `Sales` UNION SELECT*FROM `Sales2`;  # union 세로로 붙임 
...
#확인문제1. 모든 직원의 아이디, 이름, 직급,부서면을 조회하시오 
...
#확인문제3 2019년 50000이상의 매출에 대해 직원별 매출의 합이 
#          100000원 이상의 직원의 이름, 부서명, 직급, 년도, 매출합을 
#          조회하시오. 단, 매출 합이 큰 순서부터 정렬
SELECT b.`name`, c.`name`, `pos`, `year`,  
